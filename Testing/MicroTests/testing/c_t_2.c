...
val_t cow_trie_create_val(int tag,
    ...
    if (tag == 0) {
        thing->tag = 0;
        #if COW_INCLUDE_PREDS
        int_type *pred_array = edge_list_values(thing->_.node.preds);
        thing->_.node.preds->ref_ct = 1;
        thing->_.node.pred_ct = pred_ct;
        #endif
        int_type *succ_array = edge_list_values(thing->_.node.succs);
...
int_type cow_trie_insert(cow_trie_p map, val_t val, cow_trie_p *res) {
    ...
    if (free_val_bitmap) {
        ...
        int shift_amt = LVL_CAPACITY - key_or_virtual_idx;
